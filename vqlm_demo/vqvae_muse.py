...
# from .vqvae.modeling_utils import ConfigMixin, ModelMixin, register_to_config 原先为相对导入
...
class VQGANModel(ModelMixin, ConfigMixin):
    ...
    def encode(self, pixel_values, return_loss=False):
        hidden_states = self.encoder(pixel_values) # torch.Size([4, 64, 16, 16])
        hidden_states = self.quant_conv(hidden_states) # torch.Size([4, 64, 16, 16])
        quantized_states, codebook_indices, codebook_loss = self.quantize(hidden_states, return_loss) # quantized_states.shape:torch.Size([4, 64, 16, 16]) codebook_indices:torch.Size([4, 256])
...
def get_tokenizer_muse():
    ckpts_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "/mnt/data/Students/wangxt/vqvae_ckpts") # 'D:\\2024TS\\LVM\\evaluation\\vqlm_demo\\vqvae_ckpts'
